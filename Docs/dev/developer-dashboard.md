...
- Config：脱敏查看白名单 API Key，并通过完整新值替换保存；包含算法、图片生成和图片识别（`VISION_API_KEY` / `VISION_API_URL` / `VISION_MODEL`）相关配置。`VISION_API_URL` 以 `/anthropic` 结尾时，机器人图片识别会使用 Anthropic Messages 格式调用 `/v1/messages`；小米 MiMo 地址（如 `https://api.xiaomimimo.com/v1`）使用官方 OpenAI 兼容格式和 `max_completion_tokens`；其他 URL 保持 OpenAI 兼容的 `/v1/chat/completions`。
