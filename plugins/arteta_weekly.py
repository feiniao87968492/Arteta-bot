...
async def fetch_bbc_news() -> list:
    ...
    try:
        ...
        logger.warning(f"[WeeklyNews] BBC 抓取异常: {type(e).__name__}: {e}")
...
async def fetch_sky_news() -> list:
    ...
    try:
        ...
        logger.warning(f"[WeeklyNews] Sky 抓取异常: {type(e).__name__}: {e}")
...
async def fetch_guardian_news() -> list:
    """抓取 The Guardian Arsenal 页面，提取新闻标题和链接"""
    results = []
    try:
        async with httpx.AsyncClient(verify=False, timeout=15.0) as client:
            resp = await client.get(SOURCES["guardian"]["url"], headers=HEADERS, follow_redirects=True)
            if resp.status_code != 200:
                logger.warning(f"[WeeklyNews] Guardian 返回状态码 {resp.status_code}")
                return results

            html = resp.text
            articles = re.findall(
                r'<a[^>]*href="(/football/[0-9]+/[^"]+)"[^>]*>(.*?)</a>',
                html, re.DOTALL,
            )
            seen = set()
            for href, text in articles:
                title = re.sub(r'<[^>]+>', "", text).strip()
                title = re.sub(r"\s+", " ", title)
                full_url = f"https://www.theguardian.com{href}" if href.startswith("/") else href
                if title and len(title) > 15 and title not in seen:
                    seen.add(title)
                    results.append((title, full_url))

            logger.info(f"[WeeklyNews] Guardian 源抓取到 {len(results)} 条新闻")
    except Exception as e:
        logger.warning(f"[WeeklyNews] Guardian 抓取异常: {type(e).__name__}: {e}")

    return results


async def fetch_arsenal_news() -> list:
    """从所有源抓取新闻，去重合并，返回 Top 8"""
    bbc_task = fetch_bbc_news()
    sky_task = fetch_sky_news()
    guardian_task = fetch_guardian_news()
    bbc_results, sky_results, guardian_results = await asyncio.gather(
        bbc_task, sky_task, guardian_task, return_exceptions=True
    )
    ...
    seen_titles = set()
    merged = []
    for title, url in bbc_results + sky_results + guardian_results:
        key = title[:20]
    ...
    top8 = merged[:8]
    logger.info(f"[WeeklyNews] 合并后 {len(merged)} 条，选取 Top {len(top8)}")
    return top8
...
def _clean_color_tags(text: str) -> str:
    """清理颜色标记：确保 [red]/[blue] 标签在单行内闭合，否则去除。"""
    result = []
    for line in text.split('\n'):
        has_red = '[red]' in line or '[/red]' in line
        has_blue = '[blue]' in line or '[/blue]' in line
        if has_red or has_blue:
            ...
            for tag in ('red', 'blue'):
                ...
                if open_tag in line and close_tag in line:
                    # 确保开标签在闭标签之前，且只跨一行
                    open_idx = line.index(open_tag)
                    close_idx = line.index(close_tag)
                    if open_idx < close_idx:
                        valid = True
                        break
            if not valid:
                line = re.sub(r'\[/?red\]|\[/?blue\]', '', line)
        result.append(line)
    return '\n'.join(result)
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    report = _clean_color_tags(report)
